...
class PluginInstaller {
	...
	public function init() {
		...
		add_action( 'wcfio/plugin_intaller_before_plugin_activation', array( $this, 'before_plugin_activation' ) );
		add_action( 'wcfio/plugin_intaller_after_plugin_activation', array( $this, 'after_plugin_activation' ) );
	...
	public function get_partner_plugins() {
		return array(
			// array(
			// 	'name'        => esc_html__( 'WPForms', 'wcf-theme-demo-import' ),
			// 	'description' => esc_html__( 'Join 6,000,000+ professionals who build smarter forms and surveys with WPForms.', 'wcf-theme-demo-import' ),
			// 	'slug'        => 'wpforms-lite',
			// 	'required'    => false,
			// 	'preselected' => true,
			// ),
	...
	public function set_plugins() {
		$all_plugins = array_merge( $this->get_partner_plugins(), Helpers::apply_filters( 'wcfio/register_plugins', array() ) );
	...
	public function get_theme_plugins() {
		...
		$theme_plugins = array_merge( $default_plugins, Helpers::apply_filters( 'wcfio/register_plugins', array() ) );
	...
	private function get_top_partner_plugins() {
		...
		$contact_form      = [
			 'animation-addons-for-elementor/animation-addons-for-elementor.php',
			// 'wpforms/wpforms.php',
	...
	public function install_plugin_callback() {
		...
		if ( ! current_user_can( 'install_plugins' ) ) {
			wp_send_json_error( esc_html__( 'Could not install the plugin. You don\'t have permission to install plugins.', 'wcf-theme-demo-import' ) );
		}

		$slug = ! empty( $_POST['slug'] ) ? sanitize_key( wp_unslash( $_POST['slug'] ) ) : '';

		if ( empty( $slug ) ) {
			wp_send_json_error( esc_html__( 'Could not install the plugin. Plugin slug is missing.', 'wcf-theme-demo-import' ) );
		}

		// Check if the plugin is already installed and activated.
		if ( $this->is_plugin_active( $slug ) ) {
			wp_send_json_success( esc_html__( 'Plugin is already installed and activated!', 'wcf-theme-demo-import' ) );
		}
		...
		if ( $this->is_plugin_installed( $slug ) ) {
			...
			if ( ! is_wp_error( $activated ) ) {
				wp_send_json_success( esc_html__( 'Plugin was already installed! We activated it for you.', 'wcf-theme-demo-import' ) );
			} else {
		...
		if ( ! $this->filesystem_permissions_allowed() ) {
			wp_send_json_error( esc_html__( 'Could not install the plugin. Don\'t have file permission.', 'wcf-theme-demo-import' ) );
		}
		...
		if ( $upgrader->plugin_info() ) {
			...
			if ( ! is_wp_error( $activated ) ) {
				wp_send_json_success(
					esc_html__( 'Plugin installed and activated succesfully.', 'wcf-theme-demo-import' )
				);
			} else {
		...
		wp_send_json_error( esc_html__( 'Could not install the plugin. WP Plugin installer could not retrieve plugin information.', 'wcf-theme-demo-import' ) );
	}
	...
	private function activate_plugin( $plugin_filename, $slug ) {
		Helpers::do_action( 'wcfio/plugin_intaller_before_plugin_activation', $slug );

		$activated = activate_plugin( $plugin_filename );

		Helpers::do_action( 'wcfio/plugin_intaller_after_plugin_activation', $slug );
	...
	private function filesystem_permissions_allowed() {
		...
		$url   = esc_url_raw( $wcfio->get_plugin_settings_url() );
