...
class WidgetImporter {
	...
	private static function process_import_file( $file ) {
		...
		if ( ! file_exists( $file ) ) {
			return new \WP_Error(
				...
				__( 'Error: Widget import file could not be found.', 'wcf-theme-demo-import' )
			);
		}
	...
	private static function import_data( $data ) {
		...
		if ( empty( $data ) || ! is_object( $data ) ) {
			return new \WP_Error(
				...
				__( 'Error: Widget import data could not be read. Please try a different file.', 'wcf-theme-demo-import' )
			);
		}

		// Hook before import.
		Helpers::do_action( 'wcfio/widget_importer_before_widgets_import' );
		$data = Helpers::apply_filters( 'wcfio/before_widgets_import_data', $data );
		...
		foreach ( $data as $sidebar_id => $widgets ) {
			...
			if ( isset( $wp_registered_sidebars[ $sidebar_id ] ) ) {
				...
				$sidebar_message      = __( 'Sidebar does not exist in theme (moving widget to Inactive)', 'wcf-theme-demo-import' );
			}
			...
			foreach ( $widgets as $widget_instance_id => $widget ) {
				...
				if ( ! $fail && ! isset( $available_widgets[ $id_base ] ) ) {
					...
					$widget_message      = __( 'Site does not support widget', 'wcf-theme-demo-import' ); // Explain why widget not imported.
				}

				// Filter to modify settings object before conversion to array and import.
				// Leave this filter here for backwards compatibility with manipulating objects (before conversion to array below).
				// Ideally the newer wie_widget_settings_array below will be used instead of this.
				$widget = Helpers::apply_filters( 'wcfio/widget_settings', $widget ); // Object.
				...
				$widget = Helpers::apply_filters( 'wcfio/widget_settings_array', $widget );
				...
				if ( ! $fail && isset( $widget_instances[ $id_base ] ) ) {
					...
					foreach ( $single_widget_instances as $check_id => $check_widget ) {
						...
						if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
							...
							$widget_message      = __( 'Widget already exists', 'wcf-theme-demo-import' ); // Explain why widget not imported.
				...
				if ( ! $fail ) {
					...
					Helpers::do_action( 'wcfio/widget_importer_after_single_widget_import', $after_widget_import );

					// Success message.
					if ( $sidebar_available ) {
						$widget_message_type = 'success';
						$widget_message      = __( 'Imported', 'wcf-theme-demo-import' );
					}
					else {
						$widget_message_type = 'warning';
						$widget_message      = __( 'Imported to Inactive', 'wcf-theme-demo-import' );
					}
				}

				// Result for widget instance.
				$results[ $sidebar_id ]['widgets'][ $widget_instance_id ]['name']         = isset( $available_widgets[ $id_base ]['name'] ) ? $available_widgets[ $id_base ]['name'] : $id_base; // Widget name or ID if name not available (not supported by site).
				$results[ $sidebar_id ]['widgets'][ $widget_instance_id ]['title']        = ! empty( $widget['title'] ) ? $widget['title'] : __( 'No Title', 'wcf-theme-demo-import' ); // Show "No Title" if widget instance is untitled.
		...
		// Hook after import.
		Helpers::do_action( 'wcfio/widget_importer_after_widgets_import' );

		// Return results.
		return Helpers::apply_filters( 'wcfio/widget_import_results', $results );
	}
	...
	private static function available_widgets() {
		...
		return Helpers::apply_filters( 'wcfio/available_widgets', $available_widgets );
	}
	...
	private static function format_results_for_log( $results ) {
		if ( empty( $results ) ) {
			esc_html_e( 'No results for widget import!', 'wcf-theme-demo-import' );
		}
