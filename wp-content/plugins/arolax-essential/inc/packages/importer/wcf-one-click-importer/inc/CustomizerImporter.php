...
class CustomizerImporter {
	...
	public static function import( $customizer_import_file_path ) {
		...
		if ( is_wp_error( $results ) ) {
			...
			$wcfio->append_to_frontend_error_messages( $error_message );

			// Write error to log file.
			Helpers::append_to_file(
				$error_message,
				$log_file_path,
				esc_html__( 'Importing customizer settings', 'wcf-theme-demo-import' )
			);
		}
		else {
			// Add this message to log file.
			$log_added = Helpers::append_to_file(
				esc_html__( 'Customizer settings import finished!', 'wcf-theme-demo-import' ),
				$log_file_path,
				esc_html__( 'Importing customizer settings' , 'wcf-theme-demo-import' )
			);
		}
	...
	public static function import_customizer_options( $import_file_path ) {
		...
		if ( ! file_exists( $import_file_path ) ) {
			return new \WP_Error(
				...
				sprintf( /* translators: %s - file path */
					esc_html__( 'Error: The customizer import file is missing! File path: %s', 'wcf-theme-demo-import' ),
					$import_file_path
				)
			);
		}
		...
		if ( ! is_array( $data ) && ( ! isset( $data['template'] ) || ! isset( $data['mods'] ) ) ) {
			return new \WP_Error(
				...
				esc_html__( 'Error: The customizer import file is not in a correct format. Please make sure to use the correct customizer import file.', 'wcf-theme-demo-import' )
			);
		}
		if ( $data['template'] !== $template ) {
			return new \WP_Error(
				'customizer_import_wrong_theme',
				esc_html__( 'Error: The customizer import file is not suitable for current theme. You can only import customizer settings for the same theme or a child theme.', 'wcf-theme-demo-import' )
			);
		}

		// Import images.
		if ( Helpers::apply_filters( 'wcfio/customizer_import_images', true ) ) {
		...
		$use_wp_customize_save_hooks = Helpers::apply_filters( 'wcfio/enable_wp_customize_save_hooks', false );
