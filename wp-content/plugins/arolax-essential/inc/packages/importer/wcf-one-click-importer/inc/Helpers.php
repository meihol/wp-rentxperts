...
class Helpers {
	...
	public static function download_import_files( $import_file_info ) {
		...
		$import_file_info = self::apply_filters('wcfio/pre_download_import_files', $import_file_info);
		...
		if ( empty( $import_file_info['import_file_url'] ) ) {
			...
			$content_filename = self::apply_filters( 'wcfio/downloaded_content_file_prefix', 'demo-content-import-file_' ) . self::$demo_import_start_time . self::apply_filters( 'wcfio/downloaded_content_file_suffix_and_file_extension', '.xml' );
		...
		if ( ! empty( $import_file_info['import_widget_file_url'] ) ) {
			...
			$widget_filename = self::apply_filters( 'wcfio/downloaded_widgets_file_prefix', 'demo-widgets-import-file_' ) . self::$demo_import_start_time . self::apply_filters( 'wcfio/downloaded_widgets_file_suffix_and_file_extension', '.json' );
		...
		if ( ! empty( $import_file_info['import_customizer_file_url'] ) ) {
			...
			$customizer_filename = self::apply_filters( 'wcfio/downloaded_customizer_file_prefix', 'demo-customizer-import-file_' ) . self::$demo_import_start_time . self::apply_filters( 'wcfio/downloaded_customizer_file_suffix_and_file_extension', '.dat' );
		...
		// ----- Set import_theme_settings file paths -----
		// Get Redux import file as well. If defined!
		if ( ! empty( $import_file_info['import_theme_settings'] ) && is_array( $import_file_info['import_theme_settings'] ) ) {
			$redux_items = array();

			// Setup filename paths to save the Redux content.
			foreach ( $import_file_info['import_theme_settings'] as $index => $redux_item ) {
				$redux_filename = self::apply_filters( 'wcfio/downloaded_redux_file_prefix', 'demo-redux-import-file_' ) . $index . '-' . self::$demo_import_start_time . self::apply_filters( 'wcfio/downloaded_redux_file_suffix_and_file_extension', '.json' );
			...
			// Download the import_theme_settings import file.
			$downloaded_files['redux'] = $redux_items;
		}
		...
		if ( ! $wp_filesystem->put_contents( $file_path, $content ) ) {
			return new \WP_Error(
				...
				sprintf( /* translators: %1$s - br HTML tag, %2$s - file path */
					__( 'An error occurred while writing file to your server! Tried to write a file to: %1$s%2$s.', 'wcf-theme-demo-import' ),
					'<br>',
					$file_path
				)
			);
	...
	public static function data_from_file( $file_path ) {
		...
		if ( ! $data ) {
			return new \WP_Error(
				...
				sprintf( /* translators: %1$s - br HTML tag, %2$s - file path */
					__( 'An error occurred while reading a file from your server! Tried reading file from path: %1$s%2$s.', 'wcf-theme-demo-import' ),
					'<br>',
					$file_path
				)
			);
	...
	private static function check_wp_filesystem_credentials() {
		...
		if ( ! ( 'direct' === get_filesystem_method() ) ) {
			return new \WP_Error(
				...
				sprintf( /* translators: %1$s and %2$s - strong HTML tags, %3$s - HTML link to a doc page. */
					__( 'This WordPress page does not have %1$sdirect%2$s write file access. This plugin needs it in order to save the demo import xml file to the upload directory of your site. You can change this setting with these instructions: %3$s.', 'wcf-theme-demo-import' ),
					'<strong>',
					'</strong>',
		...
		if ( false === ( $creds = request_filesystem_credentials( $demo_import_page_url, '', false, false, null ) ) ) {
			return new \WP_error(
				...
				__( 'An error occurred while retrieving reading/writing permissions to your server (could not retrieve WP filesystem credentials)!', 'wcf-theme-demo-import' )
			);
		}
		...
		if ( ! WP_Filesystem( $creds ) ) {
			return new \WP_Error(
				...
				__( 'Your WordPress login credentials don\'t allow to use WP_Filesystem!', 'wcf-theme-demo-import' )
			);
		}
	...
	public static function get_log_path() {
		...
		$upload_path = self::apply_filters( 'wcfio/upload_file_path', trailingslashit( $upload_dir['path'] ) );

		$log_path = $upload_path . self::apply_filters( 'wcfio/log_file_prefix', 'log_file_' ) . self::$demo_import_start_time . self::apply_filters( 'wcfio/log_file_suffix_and_file_extension', '.txt' );
	...
	public static function register_file_as_media_attachment( $log_path ) {
		...
		$filetype  = wp_check_filetype( basename( $log_path ), self::apply_filters( 'wcfio/file_mimes', $log_mimes ) );

		// Prepare an array of post data for the attachment.
		$attachment = array(
			'guid'           => self::get_log_url( $log_path ),
			'post_mime_type' => $filetype['type'],
			'post_title'     => self::apply_filters( 'wcfio/attachment_prefix', esc_html__( 'One Click Demo Import - ', 'wcf-theme-demo-import' ) ) . preg_replace( '/\.[^.]+$/', '', basename( $log_path ) ),
			'post_content'   => '',
			'post_status'    => 'inherit',
		);
	...
	public static function get_log_url( $log_path ) {
		$upload_dir = wp_upload_dir();
		$upload_url = self::apply_filters( 'wcfio/upload_file_url', trailingslashit( $upload_dir['url'] ) );
		...
		if ( ! current_user_can( 'import' ) ) {
			wp_die(
				sprintf( /* translators: %1$s - opening div and paragraph HTML tags, %2$s - closing div and paragraph HTML tags. */
					__( '%1$sYour user role isn\'t high enough. You don\'t have permission to import demo data.%2$s', 'wcf-theme-demo-import' ),
	...
	public static function process_uploaded_files( $uploaded_files, $log_file_path ) {
		...
		$file_not_provided_error = array(
			'error' => esc_html__( 'No file provided.', 'wcf-theme-demo-import' )
		);
		...
		if ( $content_file_info && ! isset( $content_file_info['error'] ) ) {
			...
			$log_added = self::append_to_file(
				sprintf( /* translators: %s - the error message. */
					__( 'Content file was not uploaded. Error: %s', 'wcf-theme-demo-import' ),
					$content_file_info['error']
				),
				$log_file_path,
				esc_html__( 'Upload files' , 'wcf-theme-demo-import' )
			);
		}
		...
		if ( $widget_file_info && ! isset( $widget_file_info['error'] ) ) {
			...
			$log_added = self::append_to_file(
				sprintf( /* translators: %s - the error message. */
					__( 'Widget file was not uploaded. Error: %s', 'wcf-theme-demo-import' ),
		...
		if ( $customizer_file_info && ! isset( $customizer_file_info['error'] ) ) {
			...
			$log_added = self::append_to_file(
				sprintf( /* translators: %s - the error message. */
					__( 'Customizer file was not uploaded. Error: %s', 'wcf-theme-demo-import' ),
		...
		if ( $redux_file_info && ! isset( $redux_file_info['error'] ) ) {
			if ( isset( $_POST['redux_option_name'] ) && empty( $_POST['redux_option_name'] ) ) {
				...
				self::log_error_and_send_ajax_response(
					esc_html__( 'Missing Redux option name! Please also enter the Redux option name!', 'wcf-theme-demo-import' ),
					$log_file_path,
					esc_html__( 'Upload files', 'wcf-theme-demo-import' )
				);
			}
			...
			$log_added = self::append_to_file(
				sprintf( /* translators: %s - the error message. */
					__( 'Redux file was not uploaded. Error: %s', 'wcf-theme-demo-import' ),
		...
		$log_added = self::append_to_file(
			__( 'The import files were successfully uploaded!', 'wcf-theme-demo-import' ) . self::import_file_info( $selected_import_files ),
			$log_file_path,
			esc_html__( 'Upload files' , 'wcf-theme-demo-import' )
		);
	...
	public static function import_file_info( $selected_import_files ) {
		...
		sprintf( /* translators: %s - the max execution time. */
			__( 'Initial max execution time = %s', 'wcf-theme-demo-import' ),
		...
		sprintf( /* translators: %1$s - new line break, %2$s - the site URL, %3$s - the file path for content import, %4$s - the file path for widgets import, %5$s - the file path for widgets import, %6$s - the file path for redux import. */
			__( 'Files info:%1$sSite URL = %2$s%1$sData file = %3$s%1$sWidget file = %4$s%1$sCustomizer file = %5$s%1$sRedux files:%1$s%6$s', 'wcf-theme-demo-import' ),
			PHP_EOL,
			get_site_url(),
			empty( $selected_import_files['content'] ) ? esc_html__( 'not defined!', 'wcf-theme-demo-import' ) : $selected_import_files['content'],
			empty( $selected_import_files['widgets'] ) ? esc_html__( 'not defined!', 'wcf-theme-demo-import' ) : $selected_import_files['widgets'],
			empty( $selected_import_files['customizer'] ) ? esc_html__( 'not defined!', 'wcf-theme-demo-import' ) : $selected_import_files['customizer'],
			empty( $redux_file_string ) ? esc_html__( 'not defined!', 'wcf-theme-demo-import' ) : $redux_file_string
		);
	}
	...
	public static function set_demo_import_start_time() {
		self::$demo_import_start_time = date( self::apply_filters( 'wcfio/date_format_for_file_names', 'Y-m-d__H-i-s' ) );
	}
	 ...
	 * With 3.0 we changed the filter prefix from 'pt-wcfio/' to just 'wcfio/',
	 ...
	 * With 3.0 we changed the action prefix from 'pt-wcfio/' to just 'wcfio/',
	 * but we needed to make sure backwards compatibility is in place.
	...
	public static function get_plugin_page_setup_data() {
		return Helpers::apply_filters( 'wcfio/plugin_page_setup', array(
			'parent_slug' => 'themes.php',
			'page_title'  => esc_html__( 'Theme Demo Import' , 'wcf-theme-demo-import' ),
			'menu_title'  => esc_html__( 'Import Demo Data' , 'wcf-theme-demo-import' ),
			'capability'  => 'import',
			'menu_slug'   => 'wcf-theme-demo-import',
		) );
	}
