...
class Importer {
	...
	public function import_content( $import_file_path ) {
		...
		if ( strpos( ini_get( 'disable_functions' ), 'set_time_limit' ) === false ) {
			set_time_limit( Helpers::apply_filters( 'wcfio/set_time_limit_for_demo_data_import', 300 ) );
		}
		...
		if ( ! Helpers::apply_filters( 'wcfio/regenerate_thumbnails_in_content_import', true ) ) {
	...
	public function new_ajax_request_maybe( $data ) {
		...
		if ( $time > Helpers::apply_filters( 'wcfio/time_for_one_ajax_call', 25 ) ) {
			$response = array(
				'status'  => 'newAJAX',
			...
			$log_added = Helpers::append_to_file(
				__( 'New AJAX call!' , 'wcf-theme-demo-import' ) . PHP_EOL . $message,
