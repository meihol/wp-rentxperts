...
class CustomCpt_Pro {
    ...
    function setup_post_type() {
        $args = array(
            'public'    => false,
            'label'     => __( 'Post type', 'wcf-addons-pro' ),
    ...
    public function register_sub_menu() {
        add_submenu_page(
            'wcf_addons_page',
            esc_html__( 'CPT Builder', 'wcf-addons-pro' ),
            esc_html__( 'CPT Builder', 'wcf-addons-pro' ),
            'manage_options',
            'wcf-cpt-builder',
            [ $this, 'cpt_callback' ]
    ...
    public function save_global_settings() {
        ...
        if ( ! current_user_can( 'manage_options' ) ) {
            wp_send_json_error( esc_html__( 'You are not allowed to perform this action.', 'wcf-addons-pro' ) );
        }
        ...
        if ( empty( $id ) || empty( $custom_font_global ) ) {
            wp_send_json_error( esc_html__( 'Invalid data provided.', 'wcf-addons-pro' ) );
        }

        update_post_meta( $id, 'wcfcustom_cpt_global', $custom_font_global );
        wp_send_json_success( esc_html__( 'Settings updated successfully.', 'wcf-addons-pro' ) );
    }
    ...
    public function aae_list() {
        ...
        if ( ! wp_verify_nonce( $nonce, 'wcf_admin_nonce' ) ) {
            wp_send_json_error( esc_html__( 'Invalid nonce', 'wcf-addons-pro' ) );
        } 
