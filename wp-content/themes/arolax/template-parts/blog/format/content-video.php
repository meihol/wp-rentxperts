<?php
  ...
  $blog_readmore__icon = AROLAX_IMG .'/default-blog/arrow-right-1.png';
  $read_more_icon_option = arolax_option('blog_readmore__icon');
  $feature_video_url = arolax_meta_option(get_the_ID(),'feature_video_id');
  ...
  <article <?php post_class('default-blog__item-single'); ?>>
    ...
    <?php endif; ?>
      <div class="default-blog__content">
        ...
        <?php if(arolax_option('blog_readmore',1)): ?>
          <div class="cf_btn">
            <a href="<?php the_permalink(); ?>" class="wc-btn-underline"><span class="scr_only"><?php echo esc_html('Blog details page button'); ?></span><?php echo esc_html(arolax_option('blog_readmore_text','Read more')); ?><img src="<?php echo esc_url($blog_readmore__icon); ?>" alt="<?php esc_attr__('arrow right','arolax'); ?>"> </a>
          </div>
        <?php endif; ?>
      </div>
  </article>
